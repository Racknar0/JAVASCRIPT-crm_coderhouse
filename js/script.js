...
function preguntar() {
    ...
    while (
        isNaN(usuario.phoneNumber) ||
        usuario.phoneNumber < 1 ||
        usuario.phoneNumber.toString().length !== 10
    ) {
        alert(
            'Ingrese un valor Numérico Valido y con una extension nacional de 10 digitos'
        );
        usuario.phoneNumber = parseInt(
            preguntaAlUser('el telefono del usuario')
        );
    }

    usuario.email = preguntaAlUser(
        'Ingrese el correo del usuario'
    ).toLowerCase();
    usuario.brand = preguntaAlUser('Ingrese la marca del equipo').toLowerCase();
    usuario.model = preguntaAlUser(
    ...
    usuario.diagnostic = preguntaAlUser(
        'Ingrese el diagnostico del equipo',
        ''
    ).toLowerCase();
...
function mostrarResultado(usuarios) {
    /************** ( 'DOM SCRIPTING' ) **************/
